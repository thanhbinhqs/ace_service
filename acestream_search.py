...
def make_playlist(args, item, icon=''):
    # print(icon)
    if item['availability_updated_at'] >= args.after \
        ...
        if args.show_epg and 'channel_id' in item:
            title += ' tvg-id="' + str(item['channel_id']) + '"' + ' tvg-logo="'  +icon + '"'
        if icon != '':
            title += ' tvg-logo="'  +icon + '"'
        title += ',' + item['name']
        # if not args.quiet:
    ...
    for channels in get_channels(args):
        ...
        # and finally main thing: m3u playlist output
            ...
            if args.group_by_channels or 'items' in channels[0].keys():
                for group in channels:
                    icon = ''
                    if('icons' in group.keys()):
                        # print(group['icons'][0]['url'])
                        icon = group['icons'][0]['url']

                    for item in group['items']:                        
                        match = make_playlist(args, item, icon)
                        if match:
